...
public final class PendingHooks {
    ...
    public static void hookPendingMethod(Class<?> clazz) {
        ConcurrentHashMap<Executable, XposedBridge.AdditionalHookInfo> record = sPendingHooks.remove(clazz);
        if (record != null) {
            for (Map.Entry<Executable,XposedBridge.AdditionalHookInfo> hook : record.entrySet()) {
                YahfaHooker.hookMethod(hook.getKey(), (XposedBridge.AdditionalHookInfo) hook.getValue());
            }
        }
    }
    ...
    public static void recordPendingMethod(Method hookMethod,
        ...
        ConcurrentHashMap<Executable, XposedBridge.AdditionalHookInfo> pending = sPendingHooks.computeIfAbsent(hookMethod.getDeclaringClass(), aClass -> new ConcurrentHashMap<>());
