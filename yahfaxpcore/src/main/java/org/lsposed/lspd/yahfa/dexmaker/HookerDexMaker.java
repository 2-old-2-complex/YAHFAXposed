...
public class HookerDexMaker {
    ...
    private static final HashMap<Class<?>, Character> descriptors = new HashMap<Class<?>, Character>() {{
        put(int.class, 'I');
        put(boolean.class, 'Z');
        put(char.class, 'C');
        put(long.class, 'J');
    ...
    private static char[] getDescriptors(Class<?>[] classes) {
        char[] descriptors = new char[classes.length];
    ...
    private void doMake(String methodName) throws Exception {
        ...
        Field hooker = hookClass.getDeclaredField(FIELD_NAME_HOOKER);
